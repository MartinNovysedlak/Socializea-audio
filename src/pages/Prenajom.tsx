...
const Prenajom = ({ equipment }: Omit<PrenajomProps, 'quantities' | 'setQuantities'>) => {
  ...
  const handleOpenDetail = (pkg: PackageOption) => { setSelectedPackage(pkg); setIsDetailOpen(true); };
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "itemListElement": PRESET_FALLBACK.map((pkg, idx) => ({
              "@type": "ListItem",
              "position": idx + 1,
              "item": { "@type": "Product", "name": pkg.name, "description": pkg.description, "offers": { "@type": "Offer", "price": pkg.priceNoLights, "priceCurrency": "EUR", "availability": "https://schema.org/InStock" } }
            }))
          })}
        </script>
      </Helmet>
      ...
      <main className="min-h-screen bg-[#020721]" ref={mainRef} style={{ opacity: 0, transition: 'opacity 0.3s ease' }}>
        ...
        <section className="relative pt-36 pb-16 overflow-hidden bg-gradient-to-b from-[#020721] via-[#05092a] to-[#020721]">
          ...
          <div className="container mx-auto px-4 relative z-10 text-center max-w-4xl animate-fade-slide-up">
            <Badge className="bg-[#BD20D3] hover:bg-[#BD20D3]/80 text-white mb-6 px-4 py-1.5 rounded-full text-xs uppercase tracking-widest font-extrabold shadow-[0_0_15px_rgba(189,32,211,0.3)]">Výber techniky</Badge>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-tight mb-6">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#BD20D3] to-[#1A4BFF]">a svetlo na akciu</span>
            </h1>
            <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-10 leading-relaxed">Chystáte akciu? Vyberte si predpripravený komplet alebo si vyskladajte vlastnú aparatúru.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Button onClick={() => handleScrollTo('sety')} className="w-full sm:w-auto btn-cyber rounded-2xl h-14 px-8 text-base font-bold hover:scale-105 border-none shadow-[0_0_20px_rgba(189,32,211,0.4)]">
                <Layers className="mr-2 animate-pulse" size={18} /> Pozrieť hotové balíky
              </Button>
              <Button onClick={() => handleScrollTo('polozky')} variant="outline" className="w-full sm:w-auto border-white/20 hover:border-[#BD20D3] hover:text-white hover:shadow-[0_0_15px_rgba(189,32,211,0.2)] text-white bg-white/5 rounded-2xl h-14 px-8 text-base font-bold transition-all hover:scale-105">
                Vyskladať si aparatúru <ArrowDown className="ml-2 animate-bounce" size={16} />
              </Button>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4">
            <div className="text-center max-w-2xl mx-auto mb-12 md:mb-16 space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#BD20D3]/10 border border-[#BD20D3]/30 text-[#BD20D3] text-xs font-bold uppercase tracking-widest">
                <Sparkles size={14} className="animate-spin-slow" /><span>Bez starostí</span>
              </div>
              <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white">Naša ponuka balíkov</h2>
              <p className="text-gray-400 text-sm md:text-base leading-relaxed">Vyberte si jeden z našich overených a vyvážených setov, ktoré sme zostavili na základe stoviek úspešných akcií.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
              {displayedPackages.map((pkg, index) => {
                ...
                <ScrollReveal key={pkg.id} direction="up" delay={index * 0.15}>
                  <Card className="relative overflow-hidden bg-[#0e122b]/80 border border-white/10 rounded-3xl flex flex-col hover:border-[#BD20D3]/50 hover:shadow-[0_0_30px_rgba(189,32,211,0.1)] hover:-translate-y-2 transition-all duration-300 group h-full cursor-pointer" onClick={() => handleOpenDetail(pkg)}>
                    ...
                    <CardHeader className="p-6 pb-4 shrink-0">
                      <CardTitle className="text-xl sm:text-2xl font-bold text-white group-hover:text-[#BD20D3] transition-colors">{pkg.name}</CardTitle>
                      <p className="text-gray-400 text-xs md:text-sm mt-2 leading-relaxed h-14 overflow-hidden">{pkg.description}</p>
                    ...
                    <CardContent className="px-6 pb-6 pt-0 flex-1 flex flex-col">
                      <div className="border-t border-white/5 pt-4 space-y-2.5 flex-1">
                        ...
                        {pkg.soundSpecs.slice(0, 3).map((comp, idx) => (<div key={idx} className="flex items-start gap-2.5 text-xs text-gray-300"><Check size={12} className="text-[#BD20D3] shrink-0 mt-0.5" /><span>{comp}</span></div>))}
                        {remainingSound > 0 && <div className="text-xs text-gray-500 italic">+ {remainingSound} {remainingSound === 1 ? 'produkt' : 'produkty'} z kategórie zvuk</div>}
                        {pkg.lightSpecs.length > 0 && <div className="flex items-start gap-2.5 text-xs text-gray-300 mt-2 pt-2 border-t border-white/5"><Lightbulb size={12} className="text-[#1A4BFF] shrink-0 mt-0.5" /><span className="font-medium text-white">Svetlá & efekty:</span></div>}
                        {pkg.lightSpecs.slice(0, 2).map((comp, idx) => (<div key={idx} className="flex items-start gap-2.5 text-xs text-gray-300 ml-5"><Check size={12} className="text-[#1A4BFF] shrink-0 mt-0.5" /><span>{comp}</span></div>))}
                        {remainingLights > 0 && <div className="text-xs text-gray-500 italic ml-5">+ {remainingLights} {remainingLights === 1 ? 'produkt' : 'produkty'} z kategórie svetlá a efekty</div>}
                      </div>
                    </CardContent>
                    <CardFooter className="p-6 pt-0 shrink-0">
                      <Button onClick={(e) => { e.stopPropagation(); handleOpenDetail(pkg); }} className="w-full btn-cyber rounded-xl h-14 border-none font-bold text-sm">Detail balíka a rezervácia <ChevronRight className="ml-2" size={14} /></Button>
                    </CardFooter>
                  </Card>
                </ScrollReveal>
                );
        ...
        <section id="polozky" className="py-12 md:py-16 bg-[#020721]">
          <div className="container mx-auto px-4 text-center max-w-4xl mb-12">
            <Badge variant="outline" className="border-cyan-500/30 text-cyan-400 mb-4 px-3 py-1 text-xs uppercase tracking-wider font-semibold">Vlastná konfigurácia</Badge>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold text-white mb-4">Potrebujete len konkrétny kus?</h2>
            <p className="text-gray-400 text-sm md:text-base">Prehliadajte našu kompletnú ponuku a nakombinujte si reproduktory, mikrofóny alebo káble podľa seba.</p>
          </div>
          <EquipmentCatalog equipment={equipment} loading={equipment.length === 0} quantities={quantities} setQuantities={setQuantities} />
        </section>

        <PackageDetailDialog open={isDetailOpen} onOpenChange={(open) => { setIsDetailOpen(open); if (!open) setSelectedPackage(null); }} selectedPackage={selectedPackage} />
        <Footer />
      </main>
    </>
  );
