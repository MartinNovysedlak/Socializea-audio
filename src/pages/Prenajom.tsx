  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#020721]">
        ...
        <section className="relative pt-28 sm:pt-36 pb-12 sm:pb-16 overflow-hidden bg-gradient-to-b from-[#020721] via-[#05092a] to-[#020721]">
          ...
          <div className="container mx-auto px-4 relative z-10 text-center max-w-4xl animate-fade-slide-up">
            <Badge className="bg-[#BD20D3] hover:bg-[#BD20D3]/80 text-white mb-4 sm:mb-6 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs uppercase tracking-widest font-extrabold shadow-[0_0_15px_rgba(189,32,211,0.3)]">
              Výber techniky
            </Badge>
            <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold text-white tracking-tight leading-tight mb-4 sm:mb-6 px-2 sm:px-0">
            ...
            <p className="text-base sm:text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-8 sm:mb-10 leading-relaxed px-2 sm:px-0">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 px-2 sm:px-0">
              <Button 
                ...
                className="w-full sm:w-auto btn-cyber rounded-2xl h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base font-bold hover:scale-105 border-none shadow-[0_0_20px_rgba(189,32,211,0.4)]"
              >
                <Layers className="mr-2 animate-pulse shrink-0" size={16} />
                Pozrieť hotové balíky
              </Button>
              <Button 
                onClick={() => handleScrollTo('polozky')}
                variant="outline"
                className="w-full sm:w-auto border-white/20 hover:border-[#BD20D3] hover:text-white hover:shadow-[0_0_15px_rgba(189,32,211,0.2)] text-white bg-white/5 rounded-2xl h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base font-bold transition-all hover:scale-105"
              >
                Vyskladať si aparatúru
                <ArrowDown className="ml-2 animate-bounce shrink-0" size={16} />
              </Button>
            </div>
          </div>
        </section>

        <section id="sety" className="py-12 sm:py-20 bg-[#020721]/50 border-y border-white/5 relative">
          <div className="container mx-auto px-4">
            <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-16 space-y-3 sm:space-y-4">
              ...
              <h2 className="text-2xl sm:text-3xl md:text-5xl font-bold text-white">Naša ponuka balíkov</h2>
            ...
            {loadingPackages ? (
              <div className="text-center py-12 text-gray-400 text-sm">Načítavam balíky...</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
                {presetPackages.map((pkg, index) => {
                  ...
                  <ScrollReveal key={pkg.id} direction="up" delay={index * 0.15}>
                    <Card 
                      className="relative overflow-hidden bg-[#0e122b]/80 border border-white/10 rounded-2xl sm:rounded-3xl flex flex-col hover:border-[#BD20D3]/50 hover:shadow-[0_0_30px_rgba(189,32,211,0.1)] hover:-translate-y-2 transition-all duration-300 group h-full cursor-pointer"
                    ...
                    >
                      ...
                      <CardHeader className="p-4 sm:p-6 pb-3 sm:pb-4 shrink-0">
                        <CardTitle className="text-lg sm:text-xl md:text-2xl font-bold text-white group-hover:text-[#BD20D3] transition-colors">
                          {pkg.name}
                        </CardTitle>
                        <p className="text-gray-400 text-[10px] sm:text-xs md:text-sm mt-1 sm:mt-2 leading-relaxed h-10 sm:h-14 overflow-hidden">
                          {pkg.description}
                        </p>
                        <div className="flex items-baseline gap-1 sm:gap-1.5 pt-3 sm:pt-4">
                          <span className="text-xl sm:text-2xl md:text-3xl font-extrabold text-[#BD20D3]">{pkg.priceNoLights} €</span>
                          <span className="text-gray-400 text-[9px] sm:text-xs">/ deň bez svetiel</span>
                        </div>
                        <div className="flex items-baseline gap-1 sm:gap-1.5 text-xs sm:text-sm">
                          <span className="text-base sm:text-lg md:text-xl font-bold text-[#1A4BFF]">{pkg.priceWithLights} €</span>
                          <span className="text-gray-400 text-[9px] sm:text-xs">/ deň so svetlami</span>
                        </div>
                      </CardHeader>

                      <CardContent className="px-4 sm:px-6 pb-4 sm:pb-6 pt-0 flex-1 flex flex-col">
                        <div className="border-t border-white/5 pt-3 sm:pt-4 space-y-2 sm:space-y-2.5 flex-1">
                          <p className="text-[10px] sm:text-xs font-bold uppercase text-gray-400 tracking-wider">Komponenty v sete:</p>
                          {pkg.soundSpecs.slice(0, 3).map((comp, idx) => (
                            <div key={idx} className="flex items-start gap-2 sm:gap-2.5 text-[10px] sm:text-xs text-gray-300">
                          ...
                          {pkg.lightSpecs.length > 0 && (
                            <div className="flex items-start gap-2 sm:gap-2.5 text-[10px] sm:text-xs text-gray-300 mt-1 sm:mt-2 pt-2 border-t border-white/5">
                          ...
                          {pkg.lightSpecs.slice(0, 2).map((comp, idx) => (
                            <div key={idx} className="flex items-start gap-2 sm:gap-2.5 text-[10px] sm:text-xs text-gray-300 ml-4 sm:ml-5">
                              <Check size={10} className="text-[#1A4BFF] shrink-0 mt-0.5" />
                              <span>{comp}</span>
                            </div>
                          ))}
                          {remainingLights > 0 && (
                            <div className="text-[10px] sm:text-xs text-gray-500 italic ml-4 sm:ml-5">
                      ...
                      <CardFooter className="p-4 sm:p-6 pt-0 shrink-0">
                        <Button 
                          onClick={(e) => { e.stopPropagation(); handleOpenDetail(pkg); }}
                          className="w-full btn-cyber rounded-xl h-11 sm:h-14 border-none font-bold text-xs sm:text-sm"
                        >
                          Detail balíka a rezervácia
                          <ChevronRight className="ml-1 sm:ml-2 shrink-0" size={14} />
                        </Button>
                      </CardFooter>
                    </Card>
                  </ScrollReveal>
        ...
        <section id="polozky" className="py-12 sm:py-16 bg-[#020721]">
          <div className="container mx-auto px-4 text-center max-w-4xl mb-8 sm:mb-12">
            <Badge variant="outline" className="border-cyan-500/30 text-cyan-400 mb-3 sm:mb-4 px-2 sm:px-3 py-1 text-[10px] sm:text-xs uppercase tracking-wider font-semibold">
              Vlastná konfigurácia
            </Badge>
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-extrabold text-white mb-3 sm:mb-4 px-2">
