...
const Admin = () => {
  ...
  const moveItem = (index: number, direction: 'up' | 'down') => {
    ...
    if (direction === 'down' && index === equipment.length - 1) return;

    const newEquipment = [...equipment];
    const targetIndex = direction === 'up' ? index - 1 : index + 1;
    
    [newEquipment[index], newEquipment[targetIndex]] = [newEquipment[targetIndex], newEquipment[index]];
    
    // Update order in background
    const updates = newEquipment.map((item, idx) => ({
      id: item.id,
      order_index: idx
    }));
    
    equipmentService.updateOrder(updates).then(success => {
      if (success) {
        refetch();
      }
    });
              ...
              <CardHeader className="border-b border-white/10 px-6 py-5">
                <div>
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    Pre zmenu poradia použite šípky hore/dole. Kliknite na Upraviť pre editáciu produktu.
                  </p>
                </div>
              </CardHeader>
                            ...
                            <td className="px-4 py-4">
                              <div className="flex flex-col items-center gap-1">
                                <button
                                  ...
                                  disabled={index === 0}
                                  className="w-7 h-7 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                                >
                                  <ChevronUp size={14} />
                                </button>
