          ...
          <div className="bg-gradient-to-br from-[#020721] via-[#0a0d1f] to-[#020721] border border-[#BD20D3]/20 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 backdrop-blur-xl overflow-hidden relative">
            ...
            <div className="max-w-3xl mx-auto">
              <div className="text-center mb-6 md:mb-10">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#BD20D3]/10 border border-[#BD20D3]/30 text-[#BD20D3] text-[10px] md:text-xs font-bold uppercase tracking-widest mb-3 md:mb-4">
                  <HelpCircle size={12} />
                  <span>Pomocník</span>
                </div>
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white">Často kladené otázky</h2>
              ...
              <Accordion type="single" collapsible className="space-y-2 md:space-y-3">
                {faqs.map((faq) => (
                  <AccordionItem
                    key={faq.id}
                    value={faq.id}
                    className="bg-white/5 border border-white/10 rounded-xl md:rounded-2xl px-4 md:px-6 transition-all data-[state=open]:border-[#BD20D3]/30 data-[state=open]:bg-[#BD20D3]/5"
                  >
                    <AccordionTrigger className="text-white font-semibold text-left hover:text-[#BD20D3] transition-colors py-3 md:py-5 text-xs md:text-sm [&[data-state=open]>svg]:rotate-180">
                      <span className="pr-4 md:pr-8">{faq.question}</span>
                    </AccordionTrigger>
                    <AccordionContent className="text-gray-300 leading-relaxed pb-3 md:pb-5 text-xs md:text-sm">
                      {faq.answer}
                    </AccordionContent>
                  </AccordionItem>
                ))}
