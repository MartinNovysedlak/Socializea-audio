            ...
            <form onSubmit={handleBookingSubmit} className="space-y-5">
              <div className="space-y-2 text-center border-b border-white/5 pb-4">
                <h3 className="text-lg md:text-xl font-bold text-white">
                  Rezervácia: {selectedPackage.name}
                </h3>
                <p className="text-xs text-gray-400">
                  Celková cena: {totalPrice.toFixed(2)} € / víkend (
                  {includeLights ? 'so svetelnou show' : 'bez svetiel'})
                </p>
                {deliverySelected && deliveryResult && (
                  <p className="text-xs text-[#1A4BFF]">
                    Doprava do {deliveryCity}:{' '}
                    {deliveryResult.isFree ? 'Zdarma' : `${deliveryResult.price.toFixed(2)} €`}
                  </p>
                )}
                {(installSelected || installUninstallSelected) && (
                  <p className="text-xs text-[#1A4BFF]">
                    Doplnkové služby:{' '}
                    {installSelected ? 'Inštalácia' : ''}
                    {installUninstallSelected ? 'Inštalácia a deinštalácia' : ''}
                    ({(installSelected ? 20 : 0) + (installUninstallSelected ? 40 : 0)} €)
                  </p>
                )}
                {additionalProducts.length > 0 && (
                  <p className="text-xs text-[#BD20D3]">
                    + {additionalProducts.reduce((sum, p) => sum + p.quantity, 0)} ks produktov
                    navyše ({additionalProductsCost.toFixed(2)} € / víkend)
                  </p>
                )}
                <p className="text-xs text-gray-400">
                  Ponuku vám vypracujeme a pošleme obratom na e-mail.
                </p>
              </div>
              ...
              <div className="space-y-1.5">
                <Label className="text-gray-300 text-xs font-bold uppercase">
                  Poznámka / Doprava
                </Label>
                <Textarea
                  placeholder="Špecifické požiadavky, miesto konania akcie…"
              ...
              <div className="flex gap-3 pt-2">
                <Button
                  ...
                  variant="ghost"
                  onClick={resetBack}
                  className="text-xs text-gray-400 hover:text-white h-11"
                >
                  Späť
                </Button>
                <Button
                  type="submit"
                  className="btn-cyber rounded-xl h-11 flex-grow border-none font-bold"
                >
