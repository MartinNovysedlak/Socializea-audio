...
const PackageDetailDialog = ({ open, onOpenChange, selectedPackage }: PackageDetailDialogProps) => {
  ...
  const [selectedSearchItem, setSelectedSearchItem] = useState<RentalItem | null>(null);
  ...
  const confirmRentalItem = useCallback(() => {
    ...
    const maxAvailable = getAvailableForItem(selectedSearchItem.name, selectedSearchItem.availableCount);
    const usedInPkg = getUsedInPackageForDbItem(selectedSearchItem.name, packageUsedCounts);
    if (itemQuantity > maxAvailable) {
      toast.error(`Môžete pridať maximálne ${maxAvailable} ks. ${selectedSearchItem.availableCount} ks skladom, ${usedInPkg} ks už je v balíku.`);
      return;
    }
    const itemPrice = selectedSearchItem.price ?? 0;
    const existingIdx = additionalProducts.findIndex((p) => p.id === selectedSearchItem.id);
    if (existingIdx !== -1) {
      ...
      updated[existingIdx] = {
        ...updated[existingIdx],
        quantity: newQty,
        label: `${newQty} x ${selectedSearchItem.name}`,
      ...
      setAdditionalProducts((prev) => [
        ...prev,
        { id: selectedSearchItem.id, label: `${itemQuantity} x ${selectedSearchItem.name}`, quantity: itemQuantity, pricePerDay: itemPrice },
      ]);
  ...
  }, [selectedSearchItem, itemQuantity, additionalProducts, packageUsedCounts, rentalItems]);
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 bg-gradient-to-br from-[#1A4BFF]/[0.06] to-[#BD20D3]/[0.04] border border-white/[0.08] rounded-3xl overflow-hidden p-5">
            <div className="md:col-span-4">
              {allImages.length > 0 ? (
                <>
                  <div onClick={() => { setLightboxIndex(0); setLightboxOpen(true); }} className="aspect-video md:aspect-square rounded-2xl overflow-hidden bg-zinc-900 border border-white/5 cursor-pointer group relative">
                    <img src={allImages[0]} alt={selectedPackage.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                  ...
                  {allImages.length > 1 && (
                    <div className="flex gap-2 mt-2 overflow-x-auto pb-1">
                      {allImages.slice(0, 5).map((img, idx) => (
                        <button key={idx} type="button" onClick={() => { setLightboxIndex(idx); setLightboxOpen(true); }} className={`w-12 h-12 rounded-lg overflow-hidden shrink-0 border transition-all hover:border-[#BD20D3]/50 ${idx === lightboxIndex && lightboxOpen ? 'border-[#BD20D3]' : 'border-white/10'}`}>
                          <img src={img} alt="" className="w-full h-full object-cover" />
                        </button>
                      ))}
                      {allImages.length > 5 && (
                        <button type="button" onClick={() => { setLightboxIndex(5); setLightboxOpen(true); }} className="w-12 h-12 rounded-lg overflow-hidden shrink-0 border border-white/10 bg-zinc-800 flex items-center justify-center text-[10px] text-gray-400 hover:text-white transition-colors">
                          +{allImages.length - 5}
                        </button>
                      )}
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            ...
            {hasLightSection && (
              <div onClick={() => setIncludeLights(!includeLights)} className={`p-5 md:p-6 rounded-2xl border-2 transition-all flex flex-col justify-between cursor-pointer select-none group relative ${
                includeLights
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mb-4 leading-relaxed">{includeLights ? 'Svetelná show je pridaná a zahŕňa tieto položky:' : `Pridať svetelnú show a efekty? (+${lightsUpgradePrice} €)`}</p>
                  <ul className="space-y-3">
          ...
          <div className="bg-gradient-to-br from-[#1A4BFF]/[0.06] to-[#BD20D3]/[0.04] border border-white/[0.08] rounded-2xl p-5 space-y-3">
            ...
            <div onClick={() => { setInstallSelected(!installSelected); if (!installSelected) setInstallUninstallSelected(false); }} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${installSelected ? 'bg-[#1A4BFF]/10 border-[#1A4BFF]/40' : 'bg-black/20 border-white/5 hover:border-white/20'}`}>
              <div className="flex items-center gap-2.5">
            ...
            <div onClick={() => { setInstallUninstallSelected(!installUninstallSelected); if (!installUninstallSelected) setInstallSelected(false); }} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${installUninstallSelected ? 'bg-[#1A4BFF]/10 border-[#1A4BFF]/40' : 'bg-black/20 border-white/5 hover:border-white/20'}`}>
              <div className="flex items-center gap-2.5">
            ...
            <div className="border-t border-white/[0.06] pt-3 space-y-2">
                  ...
                  <div className="absolute top-full left-0 right-0 mt-0.5 bg-[#0a0d1f] border border-white/[0.12] rounded-xl shadow-2xl shadow-black/50 overflow-hidden z-50 max-h-60 overflow-y-auto">
                    {searchingCities && (
                      <div className="flex items-center justify-center gap-2 p-3 border-b border-white/[0.06] text-gray-500">
                        <Loader2 size={14} className="animate-spin" /><span className="text-xs">Vyhľadávam...</span>
                      </div>
                    )}
              ...
              {deliverySelected && (
                <button type="button" onClick={() => setMapPickerOpen(true)} className="flex items-center gap-2 text-xs text-[#1A4BFF] hover:text-[#1A4BFF]/80 transition-colors py-1.5 px-3 rounded-lg bg-[#1A4BFF]/5 hover:bg-[#1A4BFF]/10 border border-[#1A4BFF]/20 w-full justify-center">
                  <Map size={14} /> Vybrať na mape
                </button>
              )}
            ...
            <div className="relative" ref={searchRef}>
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <Input ref={searchInputRef} type="text" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setSearchOpen(true); setSelectedSearchItem(null); }} onFocus={() => setSearchOpen(true)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); if (selectedSearchItem) confirmRentalItem(); else if (filteredItems.length > 0) setSelectedSearchItem(filteredItems[0]); else if (searchTerm.trim()) addCustomProduct(); } }} placeholder="Hľadať v databáze alebo napísať vlastnú položku..." className="bg-black/40 border-white/[0.12] text-white rounded-xl h-10 pl-9 text-xs placeholder:text-gray-500 focus:border-[#1A4BFF]/50 focus:ring-[#1A4BFF]/20" />
                </div>
                <Button type="button" size="sm" onClick={() => { if (selectedSearchItem) confirmRentalItem(); else if (searchTerm.trim()) addCustomProduct(); }} disabled={!searchTerm.trim()} className="bg-[#1A4BFF]/15 hover:bg-[#1A4BFF]/30 border border-[#1A4BFF]/25 text-white rounded-xl h-10 px-3 text-xs disabled:opacity-30 transition-all">
              ...
              {searchOpen && searchTerm.trim() && (
                <>
                  ...
                  {!loadingItems && !dbError && filteredItems.length > 0 && !selectedSearchItem && (
                    <div className="absolute top-full left-0 right-24 mt-1 bg-[#0a0d1f] border border-white/[0.12] rounded-xl shadow-2xl shadow-black/50 overflow-hidden z-50">
                      {filteredItems.map((item) => {
                        ...
                        return (
                          <button key={item.id} type="button" onClick={() => { if (remaining <= 0) { toast.error(`Pre položku "${item.name}" nie sú k dispozícii ďalšie kusy.`); return; } setSelectedSearchItem(item); setItemQuantity(1); }} className={`flex items-center gap-2.5 w-full p-2.5 transition-colors text-left border-b border-white/[0.06] last:border-b-0 ${remaining > 0 ? 'hover:bg-[#1A4BFF]/5 cursor-pointer' : 'opacity-40 cursor-not-allowed'}`}>
                            <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 bg-zinc-800"><img src={item.image} alt={item.name} className="w-full h-full object-cover" /></div>
                            <div className="flex-1 min-w-0"><p className="text-xs text-white truncate">{item.name}</p>{item.category && <p className="text-[9px] text-gray-500 uppercase tracking-wider">{item.category}</p>}</div>
                  ...
                  {!loadingItems && !dbError && selectedSearchItem && (
                    <div className="absolute top-full left-0 right-24 mt-1 bg-[#0a0d1f] border border-[#1A4BFF]/30 rounded-xl p-3 shadow-2xl z-50">
                      <div className="flex items-center gap-2.5 mb-2">
                        <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 bg-zinc-800 border border-white/10"><img src={selectedSearchItem.image} alt="" className="w-full h-full object-cover" /></div>
                        <p className="text-xs font-bold text-white truncate flex-1">{selectedSearchItem.name}</p>
                      </div>
                      <div className="flex items-center justify-between gap-3 bg-black/40 border border-white/[0.08] rounded-xl p-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button type="button" onClick={() => { const maxAvail = getAvailableForItem(selectedSearchItem.name, selectedSearchItem.availableCount); setItemQuantity(Math.min(maxAvail, itemQuantity + 1)); }} disabled={itemQuantity >= getAvailableForItem(selectedSearchItem.name, selectedSearchItem.availableCount)} className="w-7 h-7 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 disabled:opacity-30 transition-all"><Plus size={12} /></button>
                        </div>
                      </div>
                      <div className="flex gap-2 mt-2">
                        <Button type="button" variant="ghost" size="sm" onClick={() => { setSelectedSearchItem(null); setItemQuantity(1); }} className="text-[10px] text-gray-400 hover:text-white h-8 flex-1">Zrušiť</Button>
          ...
          <div className="bg-gradient-to-br from-[#020721] to-[#0a0d1f] border border-[#BD20D3]/20 rounded-2xl p-5 space-y-2 shadow-[0_0_20px_rgba(189,32,211,0.05)]">
            <span className="text-xs font-bold uppercase tracking-widest text-gray-300 pb-1 flex items-center gap-1.5"><Euro size={14} className="text-[#BD20D3]" /> Súhrn cien</span>
            <div className="flex justify-between text-xs text-gray-400"><span>Balík ({includeLights ? 'so svetlami' : 'bez svetiel'}):</span><span className="text-white font-semibold">{activePackagePrice} € / víkend</span></div>
            ...
            {deliverySelected && deliveryResult && (
              <div className="flex justify-between text-xs text-gray-400"><span>Doprava ({deliveryCity}):</span><span className={deliveryResult.isFree ? 'text-emerald-400 font-semibold' : 'text-[#1A4BFF] font-semibold'}>{deliveryResult.isFree ? 'Zdarma' : `+${deliveryResult.price} €`}</span></div>
            )}
            {additionalProducts.length > 0 && (
              <>
                <div className="border-t border-white/[0.06] pt-2 mt-2 space-y-1.5">
                  ...
                  {additionalProducts.map((p) => (
                    <div key={p.id} className="flex justify-between text-[11px] text-gray-400">
                      <span className="flex items-center gap-1"><Plus size={10} className="text-[#1A4BFF]" /><span>{p.label.replace(/^\d+\s*x\s*/, '')} × {p.quantity}</span></span>
                      <span className="text-white font-medium">{(p.pricePerDay * p.quantity).toFixed(2)} €</span>
                    </div>
                  ))}
                  <div className="flex justify-between text-xs text-gray-400 border-t border-white/[0.06] pt-1">
                    ...
                    <span className="text-[#1A4BFF] font-bold">{additionalProductsCost.toFixed(2)} €</span>
                  </div>
                </div>
              </>
            )}
          ...
          <div className="flex flex-col gap-2 pt-2">
            <Button onClick={handleAddToCart} className="btn-cyber rounded-xl h-12 border-none font-bold w-full"><ShoppingBag size={16} className="mr-2" /> Pridať do košíka</Button>
            <Button variant="outline" onClick={() => onOpenChange(false)} className="border-white/10 text-white hover:bg-white/5 rounded-xl h-12 w-full">Zavrieť</Button>
            ...
            <Dialog open={questionDialogOpen} onOpenChange={setQuestionDialogOpen}>
              <DialogTrigger asChild>
                <button className="flex items-center justify-center gap-2 text-sm text-gray-400 hover:text-white transition-colors py-2 rounded-xl w-full cursor-pointer bg-transparent border-0">
                  <HelpCircle size={16} /> Je vám niečo nejasné? Spýtajte sa nás
                </button>
              </DialogTrigger>
                ...
                <form onSubmit={handleSendQuestion} className="space-y-4 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <input type="text" required value={questionFirstName} onChange={(e) => setQuestionFirstName(e.target.value)} placeholder="Napr. Ján" className="w-full bg-black/40 border border-white/10 text-white rounded-xl h-11 px-4 focus:outline-none focus:ring-1 focus:ring-[#BD20D3] text-sm" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs text-gray-400 font-bold uppercase">Priezvisko *</label>
                      <input type="text" required value={questionLastName} onChange={(e) => setQuestionLastName(e.target.value)} placeholder="Napr. Novák" className="w-full bg-black/40 border border-white/10 text-white rounded-xl h-11 px-4 focus:outline-none focus:ring-1 focus:ring-[#BD20D3] text-sm" />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs text-gray-400 font-bold uppercase">E-mail *</label>
                    <input type="email" required value={questionEmail} onChange={(e) => setQuestionEmail(e.target.value)} placeholder="jan.novak@email.sk" className="w-full bg-black/40 border border-white/10 text-white rounded-xl h-11 px-4 focus:outline-none focus:ring-1 focus:ring-[#BD20D3] text-sm" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs text-gray-400 font-bold uppercase">Telefón (voliteľný)</label>
                    <input type="tel" autoComplete="tel" value={questionPhone} onChange={(e) => setQuestionPhone(e.target.value)} placeholder="+421 901 234 567" className="w-full bg-black/40 border border-white/10 text-white rounded-xl h-11 px-4 focus:outline-none focus:ring-1 focus:ring-[#BD20D3] text-sm" />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <textarea required value={questionMessage} onChange={(e) => setQuestionMessage(e.target.value)} placeholder="Napíšte, čo vás zaujíma..." className="w-full bg-black/40 border border-white/10 text-white rounded-xl min-h-[100px] p-4 focus:outline-none focus:ring-1 focus:ring-[#BD20D3] text-sm leading-relaxed" />
                  </div>
            ...
            <Dialog open={showQuestionSuccess} onOpenChange={setShowQuestionSuccess}>
              <DialogContent className="bg-[#0a0d1f] border border-[#BD20D3]/40 text-white max-w-md rounded-3xl shadow-2xl shadow-[#BD20D3]/20 p-8 text-center">
                ...
                <DialogHeader className="space-y-3">
                  ...
                  <DialogDescription className="text-gray-300 text-base leading-relaxed">Vaša otázka bola úspešne odoslaná.</DialogDescription>
                </DialogHeader>
                <Button onClick={() => setShowQuestionSuccess(false)} className="btn-cyber border-none rounded-xl h-12 px-8 font-bold mt-6 w-full">Zavrieť</Button>
              </DialogContent>
            </Dialog>
          </div>
        </div>
      ...
      <Dialog open={lightboxOpen} onOpenChange={setLightboxOpen}>
        <DialogContent className="bg-black/95 border-white/10 max-w-4xl rounded-3xl p-0 overflow-hidden shadow-2xl shadow-black/80">
          <div className="relative flex items-center justify-center min-h-[50vh] md:min-h-[70vh]">
            <button type="button" onClick={() => setLightboxOpen(false)} className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-black/60 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all"><X size={18} /></button>
            <div className="absolute top-4 left-4 z-20 bg-black/60 border border-white/10 rounded-full px-3 py-1 text-xs text-white">{lightboxIndex + 1} / {allImages.length}</div>
            {allImages.length > 1 && (
              <button type="button" onClick={(e) => { e.stopPropagation(); setLightboxIndex((prev) => (prev > 0 ? prev - 1 : allImages.length - 1)); }} className="absolute left-3 z-20 w-10 h-10 rounded-full bg-black/60 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all"><ChevronLeft size={20} /></button>
            )}
            <img src={allImages[lightboxIndex]} alt={`${selectedPackage.name} – obrázok ${lightboxIndex + 1}`} className="w-full h-full object-contain max-h-[80vh]" />
            {allImages.length > 1 && (
              <button type="button" onClick={(e) => { e.stopPropagation(); setLightboxIndex((prev) => (prev < allImages.length - 1 ? prev + 1 : 0)); }} className="absolute right-3 z-20 w-10 h-10 rounded-full bg-black/60 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all"><ChevronRight size={20} /></button>
            )}
            {allImages.length > 1 && (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex gap-2 bg-black/60 border border-white/10 rounded-xl px-3 py-2 max-w-[90%] overflow-x-auto">
                {allImages.map((img, idx) => (
                  <button key={idx} type="button" onClick={() => setLightboxIndex(idx)} className={`w-10 h-10 rounded-lg overflow-hidden shrink-0 border-2 transition-all ${idx === lightboxIndex ? 'border-[#BD20D3]' : 'border-transparent opacity-60 hover:opacity-100'}`}>
                    <img src={img} alt="" className="w-full h-full object-cover" />
                  </button>
                ))}
              </div>
