                ...
                <div className="lg:col-span-5 space-y-4 min-w-0">
                  ...
                  <div className="space-y-3 max-h-[260px] overflow-y-auto pr-1 custom-scrollbar">
                    {cartItems.map(({ item, qty }) => {
                      ...
                      return (
                        <div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 bg-white/5 border border-white/10 rounded-xl p-3 overflow-hidden">
                          <div className="flex items-center gap-3 min-w-0 flex-1 w-full sm:w-auto">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-[#BD20D3] font-bold text-xs sm:text-sm mt-0.5">{item.price_per_day} € / deň</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 bg-black/30 border border-white/10 rounded-lg p-1.5 shrink-0 self-end sm:self-auto">
                    ...
                    {packageItems.map((pkg) => (
                      <div key={pkg.id} className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4 bg-[#BD20D3]/5 border border-[#BD20D3]/30 rounded-xl p-3 sm:p-4 relative overflow-hidden">
                        ...
                        <div className="flex-grow min-w-0 pr-6 sm:pr-0">
                          <h4 className="text-sm sm:text-base font-bold text-white mb-0.5 truncate">{pkg.name}</h4>
...
</dyad-chat-summary><dyad-write path="src/components/FloatingCart.tsx" description="Oprava layoutu kariet produktov v košíku – odstránenie horizontálneho scrollbaru a rozhádzaného zobrazenia počtu kusov.">
"use client";
...
const FloatingCart = ({ quantities, setQuantities, equipment }: FloatingCartProps) => {
  ...
  const buildCartSummaryHtml = () => {
    ...
    html += '<div style="margin-top:20px;padding-top:16px;border-top:2px solid #BD20D3;display:flex;justify-content:space-between;align-items:center;">';
    html += `<span style="color:white;font-size:18px;font-weight:700;">Celková suma</span><span style="color:#BD20D3;font-size:24px;font-weight:900;margin-left:24px;">${(grandTotal + packagesTotal).toFixed(2)} €</span></div>`;
    return html;
  };
    ...
    <>
            ...
            <div className="mb-4 w-80 bg-gradient-to-br from-[#0a0d1f]/95 to-[#020721]/95 border border-[#BD20D3]/40 rounded-2xl p-4 shadow-2xl backdrop-blur-md animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {cartItems.map(({ item, qty }) => {
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-2 text-sm text-gray-300">
                      <img src={img} alt="" className="w-8 h-8 rounded object-cover border border-white/10 shrink-0" onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=50"; }} />
                      <span className="font-semibold text-[#BD20D3] shrink-0">{qty}x</span>
                ...
                {packageItems.map((pkg) => (
                  <div key={pkg.id} className="flex items-center gap-2 text-sm text-gray-300">
                    <img src={pkg.image} alt={pkg.name} className="w-8 h-8 rounded object-cover border border-white/10 shrink-0" onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=50"; }} />
                    <span className="font-semibold text-[#BD20D3] shrink-0">1x</span>
                    <span className="truncate flex-1 min-w-0">{pkg.name}</span>
                    <span className="text-white text-xs font-semibold shrink-0">{getPackageTotal(pkg)} €</span>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
                <div className="lg:col-span-5 space-y-4 min-w-0">
                  ...
                  <div className="space-y-3 max-h-[260px] overflow-y-auto pr-1">
                    ...
                    {packageItems.map((pkg) => (
                      <div key={pkg.id} className="flex flex-col sm:flex-row items-start gap-3 bg-[#BD20D3]/5 border border-[#BD20D3]/30 rounded-xl p-3 relative overflow-hidden">
                        <button type="button" onClick={() => removePackage(pkg.id)} className="absolute top-2 right-2 w-6 h-6 rounded-full bg-red-500/80 hover:bg-red-500 flex items-center justify-center text-white z-10"><X size={12} /></button>
                        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden border border-[#BD20D3]/40 shrink-0 bg-black/40">
                          <img src={pkg.image} alt={pkg.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=100"; }} />
                        </div>
                        <div className="flex-1 min-w-0 pr-5">
                          <h4 className="text-sm sm:text-base font-bold text-white truncate">{pkg.name}</h4>
                          <p className="text-[#BD20D3] font-bold text-xs sm:text-sm mt-0.5">{getPackageTotal(pkg)} € / víkend</p>
                          <div className="flex flex-wrap gap-1.5 mt-1.5">
                  ...
                  {hasEquipment && (
                    <div className="bg-gradient-to-br from-[#1A4BFF]/[0.06] to-[#BD20D3]/[0.04] border border-white/[0.08] rounded-2xl p-4 space-y-2">
                      ...
                      <div onClick={() => { if (packageHasInstall) return; setInstallSelected(!installSelected); if (!installSelected) setInstallUninstallSelected(false); }}
                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${packageHasInstall ? 'bg-gray-800/40 border-gray-700/50 opacity-50 cursor-not-allowed' : installSelected ? 'bg-[#1A4BFF]/10 border-[#1A4BFF]/40' : 'bg-black/20 border-white/5 hover:border-white/20'}`}>
                        <div className="flex items-center gap-2.5">
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${packageHasInstall ? 'border-gray-600 bg-gray-700' : installSelected ? 'bg-[#1A4BFF] border-[#1A4BFF]' : 'border-gray-500'}`}>
                            {packageHasInstall && <Ban size={10} className="text-gray-400" />}
                      ...
                      <div onClick={() => { if (packageHasInstall) return; setInstallUninstallSelected(!installUninstallSelected); if (!installUninstallSelected) setInstallSelected(false); }}
                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${packageHasInstall ? 'bg-gray-800/40 border-gray-700/50 opacity-50 cursor-not-allowed' : installUninstallSelected ? 'bg-[#1A4BFF]/10 border-[#1A4BFF]/40' : 'bg-black/20 border-white/5 hover:border-white/20'}`}>
                        <div className="flex items-center gap-2.5">
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${packageHasInstall ? 'border-gray-600 bg-gray-700' : installUninstallSelected ? 'bg-[#1A4BFF] border-[#1A4BFF]' : 'border-gray-500'}`}>
                            {packageHasInstall && <Ban size={10} className="text-gray-400" />}
                      ...
                      <div className="border-t border-white/[0.06] pt-2 space-y-2">
                        <div className="relative" ref={cityRef}>
                          <div onClick={() => { if (packageHasDelivery) return; toggleDelivery(); }}
                            className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${packageHasDelivery ? 'bg-gray-800/40 border-gray-700/50 opacity-50 cursor-not-allowed' : 'bg-black/20 border-white/5 hover:border-white/20'}`}>
                            <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all shrink-0 ${packageHasDelivery ? 'border-gray-600 bg-gray-700' : deliverySelected ? 'bg-[#1A4BFF] border-[#1A4BFF]' : 'border-gray-500'}`}>
                              {packageHasDelivery && <Ban size={10} className="text-gray-400" />}
                              {!packageHasDelivery && deliverySelected && <Check size={12} className="text-white stroke-[3]" />}
      ...
      <Dialog open={showReservationSuccess} onOpenChange={setShowReservationSuccess}>
        <DialogContent className="bg-[#0a0d1f] border border-[#BD20D3]/40 text-white max-w-md rounded-3xl shadow-2xl shadow-[#BD20D3]/20 p-8 text-center">
          ...
          <DialogHeader className="space-y-3">
            <DialogTitle className="text-2xl font-bold text-white">Ďakujeme za vašu rezerváciu.</DialogTitle>
            <DialogDescription className="text-gray-300 text-base leading-relaxed">Vašu rezerváciu sme prijali a čoskoro vás budeme kontaktovať pre potvrdenie termínu a finálnu kalkuláciu.</DialogDescription>
          </DialogHeader>
