...
const FloatingCart = ({ quantities, setQuantities, equipment }: FloatingCartProps) => {
  ...
  const [formData, setFormData] = useState({
    firstName: "", lastName: "", email: "", phone: "", dateFrom: "", dateTo: "", message: ""
  });
  ...
  useEffect(() => {
    try { localStorage.setItem(PACKAGE_STORAGE_KEY, JSON.stringify(packageItems)); } catch {}
  }, [packageItems]);
  ...
  useEffect(() => {
    const handler = (e: CustomEvent) => {
      ...
      if (pkg.install === 'install' || pkg.install === 'install_uninstall') {
        setInstallSelected(false); setInstallUninstallSelected(false);
      }
  ...
  useEffect(() => {
    if (!deliverySelected || !debouncedCitySearch.trim() || debouncedCitySearch.trim().length < 2 || cityLocked) {
      setCitySuggestions([]); setCityDropdownOpen(false); setSearchingCities(false);
      return;
    }
    let cancelled = false;
    const search = async () => {
  ...
  const selectCity = (cityName: string, lat: number, lng: number) => {
    ...
    if (result) {
      if (result.isFree) toast.success(`Doprava do ${cityName} je ZDARMA!`);
                ...
                <div className="lg:col-span-5 space-y-4">
                  ...
                  <div className="space-y-3 max-h-[260px] overflow-y-auto pr-2 custom-scrollbar">
                    {cartItems.map(({ item, qty }) => {
                      ...
                      return (
                        <div key={item.id} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl p-3 md:p-4">
                          <div className="w-14 h-14 rounded-lg overflow-hidden border border-white/10 shrink-0 bg-black/40"><img src={displayImg} alt={item.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=100"; }} /></div>
                          <div className="flex-1 min-w-0"><h4 className="text-base font-semibold text-white truncate">{item.name}</h4><p className="text-[#BD20D3] font-bold text-sm mt-1">{item.price_per_day} € / deň</p></div>
                    ...
                    {packageItems.map((pkg) => (
                      <div key={pkg.id} className="flex items-start gap-4 bg-[#BD20D3]/5 border border-[#BD20D3]/30 rounded-xl p-3 md:p-4 relative">
                        ...
                        <div className="w-16 h-16 rounded-lg overflow-hidden border border-[#BD20D3]/40 shrink-0 bg-black/40"><img src={pkg.image} alt={pkg.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=100"; }} /></div>
                      ...
                      <div
                        ...
                        <div className="flex items-center gap-2.5">
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${packageHasInstall ? 'border-gray-600 bg-gray-700' : installSelected ? 'bg-[#1A4BFF] border-[#1A4BFF]' : 'border-gray-500'}`}>
                            {packageHasInstall && <Ban size={10} className="text-gray-400" />}{!packageHasInstall && installSelected && <Check size={12} className="text-white stroke-[3]" />}
                          </div>
                      ...
                      <div
                        ...
                        <div className="flex items-center gap-2.5">
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${packageHasInstall ? 'border-gray-600 bg-gray-700' : installUninstallSelected ? 'bg-[#1A4BFF] border-[#1A4BFF]' : 'border-gray-500'}`}>
                            {packageHasInstall && <Ban size={10} className="text-gray-400" />}{!packageHasInstall && installUninstallSelected && <Check size={12} className="text-white stroke-[3]" />}
                          </div>
                        ...
                        <div className="relative" ref={cityRef}>
                          <div
                            ...
                            <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all shrink-0 ${packageHasDelivery ? 'border-gray-600 bg-gray-700' : deliverySelected ? 'bg-[#1A4BFF] border-[#1A4BFF]' : 'border-gray-500'}`}>
                              {packageHasDelivery && <Ban size={10} className="text-gray-400" />}{!packageHasDelivery && deliverySelected && <Check size={12} className="text-white stroke-[3]" />}
                            </div>
                  ...
                  <div className="border-t border-white/10 pt-4 space-y-2">
                    ...
                    {(installSelected && !installUninstallSelected) && (
                      <div className="flex justify-between text-base text-gray-400"><span className="flex items-center gap-1.5"><Wrench size={14} className="text-[#1A4BFF]" /> Inštalácia</span><span className="text-[#1A4BFF] font-semibold">+{installCost + packageItems.filter(p => p.install === 'install').reduce((s, p) => s + p.installPrice, 0)} €</span></div>
                    )}
                    {(installUninstallSelected && !installSelected) && (
                      <div className="flex justify-between text-base text-gray-400"><span className="flex items-center gap-1.5"><Wrench size={14} className="text-[#1A4BFF]" /> Inštalácia a deinštalácia</span><span className="text-[#1A4BFF] font-semibold">+{installUninstallCost + packageItems.filter(p => p.install === 'install_uninstall').reduce((s, p) => s + p.installPrice, 0)} €</span></div>
                    )}
                    {!installSelected && !installUninstallSelected && packageInstallCount > 0 && (
                      <div className="flex justify-between text-base text-gray-400"><span className="flex items-center gap-1.5"><Wrench size={14} className="text-[#1A4BFF]" /> Inštalácia</span><span className="text-[#1A4BFF] font-semibold">+{packageItems.filter(p => p.install === 'install').reduce((s, p) => s + p.installPrice, 0)} €</span></div>
                    )}
                    {!installSelected && !installUninstallSelected && packageInstallUninstallCount > 0 && (
                      <div className="flex justify-between text-base text-gray-400"><span className="flex items-center gap-1.5"><Wrench size={14} className="text-[#1A4BFF]" /> Inštalácia a deinštalácia</span><span className="text-[#1A4BFF] font-semibold">+{packageItems.filter(p => p.install === 'install_uninstall').reduce((s, p) => s + p.installPrice, 0)} €</span></div>
                    )}
