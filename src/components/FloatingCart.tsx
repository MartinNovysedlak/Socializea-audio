...
function calculateDelivery(
  ...
  cityName: string
): { distance: number; nearestPoint: string; isKysuce: boolean; isFree: boolean; price: number } | null {
  ...
  const price = isFree ? 0 : Math.round((minDist - 10) * 0.7);
...
const FloatingCart = ({ quantities, setQuantities, equipment }: FloatingCartProps) => {
  ...
  const getSubtotalPerDay = () =>
    cartItems.reduce((sum, { item, qty }) => sum + item.price_per_day * qty, 0);
  ...
  const buildCartSummaryHtml = () => {
    ...
    if (cartItems.length > 0) {
      html +=
        '<h3 style="color:#BD20D3;font-size:16px;margin:20px 0 10px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:8px;">🎧 Aparatúra</h3>';
    ...
    if (packageItems.length > 0) {
      html +=
        '<h3 style="color:#BD20D3;font-size:16px;margin:20px 0 10px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:8px;">📦 Balíky</h3>';
      packageItems.forEach((pkg) => {
        ...
        if (pkg.hasLights)
          html += `<span style="display:inline-block;margin-top:6px;padding:3px 10px;background:rgba(189,32,211,0.1);border:1px solid rgba(189,32,211,0.3);border-radius:8px;color:#BD20D3;font-size:11px;font-weight:600;">💡 So svetlami</span>`;
        if (pkg.install === "install")
          html += `<span style="display:inline-block;margin-top:6px;margin-left:6px;padding:3px 10px;background:rgba(26,75,255,0.1);border:1px solid rgba(26,75,255,0.3);border-radius:8px;color:#1A4BFF;font-size:11px;font-weight:600;">🔧 Inštalácia (+${pkg.installPrice} €)</span>`;
        if (pkg.install === "install_uninstall")
          html += `<span style="display:inline-block;margin-top:6px;margin-left:6px;padding:3px 10px;background:rgba(26,75,255,0.1);border:1px solid rgba(26,75,255,0.3);border-radius:8px;color:#1A4BFF;font-size:11px;font-weight:600;">🔧 Inšt.+Deinšt. (+${pkg.installPrice} €)</span>`;
        if (pkg.arrival)
          html += `<span style="display:inline-block;margin-top:6px;margin-left:6px;padding:3px 10px;background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.3);border-radius:8px;color:#10b981;font-size:11px;font-weight:600;">📍 ${pkg.arrival.name}${pkg.deliveryPrice > 0 ? ` (+${pkg.deliveryPrice} €)` : " (Zdarma)"}</span>`;
    ...
    if (installSelected || installUninstallSelected) {
      html +=
        '<h3 style="color:#BD20D3;font-size:16px;margin:20px 0 10px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:8px;">🔧 Doplnkové služby</h3>';
      if (installSelected)
        html += `<div style="display:flex;justify-content:space-between;padding:6px 0;font-size:13px;"><span style="color:#9ca3af;">Inštalácia</span><span style="color:#1A4BFF;font-weight:700;margin-left:24px;">+20 €</span></div>`;
      if (installUninstallSelected)
        html += `<div style="display:flex;justify-content:space-between;padding:6px 0;font-size:13px;"><span style="color:#9ca3af;">Inštalácia a deinštalácia</span><span style="color:#1A4BFF;font-weight:700;margin-left:24px;">+40 €</span></div>`;
    }

    if (deliveryResult) {
      html +=
        '<h3 style="color:#BD20D3;font-size:16px;margin:20px 0 10px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:8px;">🚚 Doprava</h3>';
    ...
    if (hasEquipment) {
      html +=
        '<h3 style="color:#BD20D3;font-size:16px;margin:20px 0 10px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:8px;">📅 Obdobie prenájmu</h3>';
  ...
  const hasAnyAdditionalService =
    installSelected ||
    installUninstallSelected ||
    deliverySelected ||
    packageHasInstall ||
    packageHasInstallUninstall ||
    packageItems.some((p) => p.arrival);
            ...
            <div className="mb-4 w-80 bg-gradient-to-br from-[#0a0d1f]/95 to-[#020721]/95 border border-[#BD20D3]/40 rounded-2xl p-4 shadow-2xl backdrop-blur-md animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              <div className="space-y-2 cart-scroll-wrapper">
                {cartItems.map(({ item, qty }) => {
                  const img =
                    item.main_image || (item.images && item.images[0]) || "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=50";
                  return (
                      ...
                      <img
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src =
                            "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=50";
                        }}
                      />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 lg:gap-8">
                <div className="lg:col-span-5 space-y-4">
                  ...
                  <div className="cart-scroll-wrapper space-y-3">
                    {cartItems.map(({ item, qty }) => {
                      const displayImg =
                        item.main_image || (item.images && item.images[0]) || "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=100";
                      return (
                    ...
                    {packageItems.map((pkg) => (
                      <div
                        key={pkg.id}
                        className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4 bg-[#BD20D3]/5 border border-[#BD20D3]/30 rounded-xl p-3 sm:p-4 relative"
                      >
                        <button
                          type="button"
                          onClick={() => removePackage(pkg.id)}
                          className="absolute top-3 right-3 w-6 h-6 rounded-full bg-red-500/80 hover:bg-red-500 flex items-center justify-center text-white z-10"
                        >
                          <X size={12} />
                        </button>
                        ...
                        <div className="flex-grow min-w-0 pr-6 sm:pr-0">
                          ...
                          <p className="text-[#BD20D3] font-bold text-sm sm:text-base mt-0.5 mb-1.5">
                            {getPackageDisplayTotal(pkg)} € {days > FREE_DAYS ? `(${days} dní)` : "/ 4 dni"}
                          </p>
                          <div className="flex flex-wrap gap-1.5 mt-1.5">
                            {pkg.hasLights && (
                              <span className="text-[10px] sm:text-xs px-2 py-0.5 sm:py-1 bg-[#BD20D3]/10 border border-[#BD20D3]/30 rounded-lg text-[#BD20D3] flex items-center gap-1 font-medium">
                                <Lightbulb size={11} /> So svetlami
                              </span>
                            )}
                            {pkg.install === "install" && (
                              <span className="text-[10px] sm:text-xs px-2 py-0.5 sm:py-1 bg-[#1A4BFF]/10 border border-[#1A4BFF]/30 rounded-lg text-[#1A4BFF] flex items-center gap-1 font-medium">
                                <Wrench size={11} /> Inštalácia (+{pkg.installPrice} €)
                              </span>
                            )}
                            {pkg.install === "install_uninstall" && (
                              <span className="text-[10px] sm:text-xs px-2 py-0.5 sm:py-1 bg-[#1A4BFF]/10 border border-[#1A4BFF]/30 rounded-lg text-[#1A4BFF] flex items-center gap-1 font-medium">
                                <Wrench size={11} /> Inšt.+Deinšt. (+{pkg.installPrice} €)
                              </span>
                            )}
                            {pkg.arrival && (
                              <span className="text-[10px] sm:text-xs px-2 py-0.5 sm:py-1 bg-emerald-500/10 border border-emerald-500/30 rounded-lg text-emerald-400 flex items-center gap-1 font-medium">
                                <MapPin size={11} /> {pkg.arrival.name}
                  ...
                  {hasSomethingToShow && (
                    <div className="bg-gradient-to-br from-[#1A4BFF]/[0.06] to-[#BD20D3]/[0.04] border border-white/[0.08] rounded-2xl p-4 space-y-2">
                      <span className="text-xs font-bold uppercase tracking-widest text-[#1A4BFF] flex items-center gap-1.5 pb-2 border-b border-white/[0.06]">
                        <Wrench size={14} /> Doplnkové služby
                      </span>

                      <div
                        onClick={() => {
                          if (packageHasInstall) return;
                          setInstallSelected(!installSelected);
                          if (!installSelected) setInstallUninstallSelected(false);
                        }}
                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${
                          packageHasInstall
                            ? "bg-gray-800/40 border-gray-700/50 opacity-50 cursor-not-allowed"
                            : installSelected
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          >
                            ...
                            {!packageHasInstall && installSelected && (
                              <Check size={12} className="text-white stroke-[3]" />
                            )}
                          </div>
                          <span
                            className={`text-xs font-medium ${packageHasInstall ? "text-gray-500" : "text-gray-300"}`}
                          >
                            {packageHasInstall ? "Inštalácia (už v balíku)" : "Inštalácia"}
                          </span>
                        </div>
                        <span
                          className={`text-xs font-bold ${
                            packageHasInstall ? "text-gray-600" : installSelected ? "text-[#1A4BFF]" : "text-gray-500"
                          }`}
                        >
                          {packageHasInstall ? "—" : "+20 €"}
                        </span>
                      </div>
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          >
                            {packageHasInstall && <Ban size={10} className="text-gray-400" />}
                            {!packageHasInstall && installUninstallSelected && (
                              <Check size={12} className="text-white stroke-[3]" />
                            )}
                          </div>
                          <span
                            className={`text-xs font-medium ${packageHasInstall ? "text-gray-500" : "text-gray-300"}`}
                          >
                            {packageHasInstall ? "Inštalácia a deinštalácia (už v balíku)" : "Inštalácia a deinštalácia"}
                          </span>
                        </div>
                        <span
                          className={`text-xs font-bold ${
                            packageHasInstall ? "text-gray-600" : installUninstallSelected ? "text-[#1A4BFF]" : "text-gray-500"
                          }`}
                        >
                          {packageHasInstall ? "—" : "+40 €"}
                        </span>
                      </div>
                      ...
                      <div className="border-t border-white/[0.06] pt-2 space-y-2">
                          ...
                          >
                            ...
                            <div className="shrink-0 min-w-[70px] text-right">
                              {packageHasDelivery ? (
                                ...
                                <span
                                  className={`text-xs font-bold ${deliveryResult.isFree ? "text-emerald-400" : "text-[#1A4BFF]"}`}
                                >
                                  ...
                                  >
                                    ...
                                    <div className="text-right shrink-0">
                                      ...
                                      {city.distToNearest !== undefined && city.distToNearest > 0 && (
                                        <span className="text-[10px] text-gray-600 mt-0.5 block whitespace-nowrap">
                                          ~{city.distToNearest} km od {city.nearestPoint}
                                        </span>
                                      )}
                                      {city.distToNearest !== undefined && city.distToNearest <= 0 && (
                            ...
                            <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
                              ...
                              {!deliveryResult.isFree && (
                                <span className="text-gray-400 flex items-center gap-1">
                                  <Navigation size={10} /> {deliveryResult.distance} km od {deliveryResult.nearestPoint}
                                </span>
                              )}
                        ...
                        <p className="text-[10px] text-gray-500 leading-relaxed">
                          Osobný odber v Žiline alebo Čadci je zadarmo. Doprava do 10 km od výdajných miest a po celých
                          Kysuciach je bezplatná. Nad 10 km účtujeme 0,70 € / km.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="border-t border-white/10 pt-4 space-y-4">
                    {cartItems.length > 0 && (
                      <div>
                        <h4 className="text-sm font-bold uppercase text-white/80 mb-3 flex items-center gap-2">
                          <span className="w-1.5 h-4 bg-[#BD20D3] rounded-full"></span>Aparatúra
                        </h4>
                          ...
                          {cartItems.map(({ item, qty }) => (
                            <div key={item.id} className="flex justify-between text-sm text-gray-400">
                              ...
                              <span className="text-white font-medium shrink-0">
                                {(item.price_per_day * qty).toFixed(2)} €
                              </span>
                            </div>
                          ))}
                        </div>
                        ...
                        {days > 1 && (
                          <div className="mt-2 bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-3 space-y-1.5 text-xs">
                            <p className="text-emerald-400 font-bold uppercase tracking-wider mb-1">
                              Výpočet na {days} {days === 1 ? "deň" : days < 5 ? "dni" : "dní"}
                            </p>
                            ...
                            <div className="flex justify-between text-gray-300">
                              <span>
                                {days - 1} {days - 1 === 1 ? "ďalší deň" : "ďalšie dni"} (50%)
                              </span>
                    ...
                    {packageItems.length > 0 && (
                      <div>
                        <h4 className="text-sm font-bold uppercase text-white/80 mb-3 flex items-center gap-2">
                          <span className="w-1.5 h-4 bg-[#1A4BFF] rounded-full"></span>Balíky
                        </h4>
                          ...
                          {packageItems.map((pkg) => (
                            <div key={pkg.id} className="bg-black/20 rounded-lg p-3 space-y-1.5">
                              <div className="flex justify-between items-start">
                                ...
                                <span className="text-[#BD20D3] font-bold text-sm ml-4 shrink-0">
                                  {getPackageDisplayTotal(pkg).toFixed(2)} €
                                </span>
                              </div>
                              <div className="flex flex-wrap gap-1">
                                {pkg.hasLights && (
                                  <span className="text-[10px] px-1.5 py-0.5 bg-[#BD20D3]/10 border border-[#BD20D3]/20 rounded text-[#BD20D3]">
                                    💡 So svetlami
                                  </span>
                                )}
                                {pkg.install === "install" && (
                                  <span className="text-[10px] px-1.5 py-0.5 bg-[#1A4BFF]/10 border border-[#1A4BFF]/20 rounded text-[#1A4BFF]">
                                    🔧 Inštalácia
                                  </span>
                                )}
                                {pkg.install === "install_uninstall" && (
                                ...
                                {pkg.arrival && (
                                  <span className="text-[10px] px-1.5 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded text-emerald-400">
                                    📍 {pkg.arrival.name}
                                  </span>
                                )}
                              </div>
                        ...
                        {days > FREE_DAYS && (
                          <div className="mt-2 bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-3 space-y-1.5 text-xs">
                            ...
                            <div className="flex justify-between text-gray-300">
                              <span>Víkend (pia-po) v cene</span>
                              <span className="text-white font-semibold">{packagesTotalWithoutExtra.toFixed(2)} €</span>
                            </div>
                            <div className="flex justify-between text-gray-300">
                              <span>
                                {days - FREE_DAYS} {days - FREE_DAYS === 1 ? "ďalší deň" : "ďalšie dni"} (+50 %)
                              </span>
                              <span className="text-emerald-400 font-semibold">
                                +{packagesExtraDaysTotal.toFixed(2)} €
                              </span>
                            </div>
                            <div className="flex justify-between text-emerald-400 font-semibold border-t border-emerald-500/20 pt-1.5 mt-1">
                    ...
                    {hasAnyAdditionalService && (
                      <div>
                        <h4 className="text-sm font-bold uppercase text-white/80 mb-3 flex items-center gap-2">
                          <span className="w-1.5 h-4 bg-emerald-400 rounded-full"></span>Doplnkové služby
                        </h4>
                        <div className="space-y-1.5">
                            ...
                            packageInstallCount > 0 && (
                              <div className="flex justify-between text-sm text-gray-400">
                                ...
                                <span className="text-[#1A4BFF] font-semibold">
                                  +
                                  {packageItems
                                    .filter((p) => p.install === "install")
                                    .reduce((s, p) => s + p.installPrice, 0)}
                                  €
                                </span>
                              </div>
                            )}
                          {!installSelected &&
                            !installUninstallSelected &&
                            packageInstallUninstallCount > 0 && (
                              <div className="flex justify-between text-sm text-gray-400">
                                <span>Inštalácia a deinštalácia (z balíkov)</span>
                                <span className="text-[#1A4BFF] font-semibold">
                                  +
                                  {packageItems
                                    .filter((p) => p.install === "install_uninstall")
                                    .reduce((s, p) => s + p.installPrice, 0)}
                                  €
                                </span>
                              </div>
                          ...
                          {deliverySelected && deliveryResult && (
                            <div className="flex justify-between text-sm text-gray-400">
                              ...
                              <span
                                className={`${deliveryResult.isFree ? "text-emerald-400 font-semibold" : "text-[#1A4BFF] font-semibold"}`}
                              >
                                {deliveryResult.isFree ? "Zdarma" : `+${deliveryResult.price} €`}
                              </span>
                            </div>
                          ...
                          {packageItems
                            ...
                            .map((pkg) => (
                              <div key={pkg.id} className="flex justify-between text-sm text-gray-400">
                                ...
                                <span
                                  className={`${pkg.deliveryPrice > 0 ? "text-[#1A4BFF] font-semibold" : "text-emerald-400 font-semibold"}`}
                                >
                                  {pkg.deliveryPrice > 0 ? `+${pkg.deliveryPrice} €` : "Zdarma"}
                                </span>
                              </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                      <div className="space-y-1.5">
                        <Label
                          htmlFor="firstName"
                          className="text-gray-300 flex items-center gap-1.5 text-sm"
                        >
                        ...
                        {showToCalendar && (
                          <div className="absolute top-full left-0 mt-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200 rounded-xl">
                            <DayPicker
                              ...
                              disabled={[
                                {
                                  before: formData.dateFrom
                                    ? addDays(new Date(formData.dateFrom), 1)
                                    : startOfDay(new Date()),
                                },
                              ]}
                              weekStartsOn={1}
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        <>
                          <Send size={16} /> Odoslať nezáväznú rezerváciu
                        </>
                      )}
                    </Button>
                  </form>
