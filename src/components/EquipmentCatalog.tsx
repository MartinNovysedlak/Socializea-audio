    ...
    <section className="py-8 md:py-12 bg-transparent relative">
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          <div className="bg-gradient-to-br from-[#020721] via-[#0a0d1f] to-[#020721] border border-[#BD20D3]/20 rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6 lg:p-8 backdrop-blur-xl overflow-hidden relative">
                  ...
                  <div className="flex gap-1 flex-nowrap">
                    {["all", "sound", "lighting", "other"].map((filter) => (
                      <button
                        ...
                        className={cn(
                          "px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-medium transition-all whitespace-nowrap shrink-0",
            ...
            {filteredEquipment.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                {filteredEquipment.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 md:p-6 flex flex-col items-center text-center hover:border-[#BD20D3]/30 hover:translate-y-[-4px] transition-all duration-300 group">
                      <Link to={`/equipment/${item.id}`} className="w-full flex flex-col items-center mb-4 cursor-pointer">
                        <div className="w-24 h-24 md:w-32 md:h-32 rounded-2xl overflow-hidden border border-white/10 relative mb-4">
                          <img
                            src={displayImage}
                      ...
                      <div className="flex-1 w-full flex flex-col justify-end">
                        <div className="flex flex-col md:flex-row justify-center items-center gap-1 md:gap-3 mb-4">
                          <span className="text-xl md:text-2xl font-bold text-[#BD20D3]">{item.price_per_day} €</span>
                          <span className="text-gray-400 text-xs">Dostupné: {getAvailabilityText(item.available)}</span>
                        </div>
                        ...
                        {inCartQty === 0 ? (
                          <Button
                            ...
                            size="sm"
                            className="w-full bg-[#BD20D3] hover:bg-[#BD20D3]/85 text-white rounded-lg h-9 md:h-10 mb-4 transition-all text-xs md:text-sm"
                          >
                          ...
                          <Button
                            ...
                            size="sm"
                            className="w-full btn-cyber hover:opacity-95 text-white rounded-lg h-9 md:h-10 mb-4 transition-all border-none text-xs md:text-sm"
                          >
                        ...
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                          >
                            <Minus size={11} />
                          </button>
                          <span className="w-8 md:w-10 text-center text-white font-medium text-sm md:text-base">{quantities[item.id] ?? 0}</span>
                          <button
                            onClick={() => handleQuantityChange(item.id, 1)}
